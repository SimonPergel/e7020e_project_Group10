...
use {
    ...
    // Configuration of how time, duration and timestamps will be represented
...
mod app {
    ...
    fn init(cx: init::Context) -> (Shared, Local, init::Monotonics) {
        ...
        (Shared { duty: 50, period_ms: 1000, running: true}, Local { led1, led_mirror, usb_dev, serial, buf:[0u8; 64]}, init::Monotonics(mono))
    }
    ...
    // binds = USBD, this run auto. when the USB controller detects avtivity
    #[task(binds = USBD, priority = 1, shared = [duty, period_ms, running], local = [ usb_dev, serial, buf, len: usize = 0, data_arr: [u8; 10] = [0; 10]])]
